// CS 1450 Spring202
// File used to demonstrate using the debugger to find null pointer exceptionpackage assignments;
...
public class CS1450_Assignment4_Debugging_Spring2020 {
	...
	public static void main(String[] args) throws IOException {
		...
		PinballMachine4d pinballMachine = new PinballMachine4d(numberRows, numberColumns);
		...
		while (targetsFile.hasNext()) {
			...
			Target4d target = new Target4d(type, number, points);			
	...
	public static void printReport (PinballMachine4d pinballMachine) {

		// Create an array list to store a report for each target in the pinball machine
		ArrayList<TargetReport4d> reports = new ArrayList<>();
		...
		for (int row = 0; row < pinballMachine.getNumberRows(); row++) {
			...
			for (int column = 0; column < pinballMachine.getNumberColumns(); column++) {
				...
				// Get the next target
				Target4d target = pinballMachine.getTarget(row, column);

				// If there is a target in the current row,column create a report for the target
				// !!!! ERROR HERE !!!!
				// There should be code here to ensure the plane is not null!!!
				// We discover this when using the debugger because when we try to access getType
				// that call is being made on a null value
				...
				if (target != null) {
					...
					TargetReport4d report = new TargetReport4d(row, column, type, id, points, hits);
					reports.add(report);
				}
			} // for each column
		} // for each row
	...
	public static void play(PinballMachine4d pinballMachine) throws IOException {
		...
		while (playFile.hasNext()) {
			...
			Target4d target = pinballMachine.getTarget(rowNumber, columnNumber);
...
class TargetReport4d implements Comparable<TargetReport4d> { 
	...
	public TargetReport4d (int rowNumber, int columnNumber, String type, int id, int points, int hits) {
	...
	public int compareTo(TargetReport4d otherReport) {
...
class PinballMachine4d {
	...
	public PinballMachine4d (int numberRows, int numberColumns) {
		...
		playingField = new Target4d[numberRows][numberColumns];
	}
	...
	public void addTargetToPlayingField (int row, int column, Target4d target) {
	...
	public Target4d getTarget(int row, int column) {
	...
	public void displayPlayingField () {
		...
		for (int row = 0; row < numberRows; row++) {
			...
			for (int column = 0; column < numberColumns; column ++) {
				...
				Target4d target = playingField[row][column];
...
class Target4d {
	...
	// Create a target
	public Target4d (String type, int id, int points) {
		this.type = type;
		this.id = id;
		this.points = points;
		this.hits = 0;
